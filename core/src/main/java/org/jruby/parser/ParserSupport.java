...
public class ParserSupport {
    ...
    public Node arg_concat(Node node1, Node node2) {
        return node2 == null ? node1 : new ArgsCatNode(getLine(node1), node1, node2);
    }
    ...
    public Node addRootNode(Node topOfAST) {
        ...
        return new RootNode(line, result.getScope(), topOfAST, lexer.getFile(), endPosition, coverageData != null);
    }
    ...
    public Node aryset(Node receiver, Node index) {
        ...
        return new_attrassign(receiver, CommonByteLists.ASET_METHOD, index, false);
    }
    ...
    public Node arg_add(int line, Node node1, Node node2) {
        if (node1 == null) {
            if (node2 == null) {
                return new ArrayNode(line).add(NilImplicitNode.NIL);
            } else {
                return new ArrayNode(node2);
            }
        }
    ...
    private Node cond0(Node node) {
        ...
        if (node == null) return new NilNode(lexer.getLine());
        ...
        case DREGEXPNODE: {
            ...
            return new Match2Node(line, node, new GlobalVarNode(line, symbolID(lexer, DOLLAR_UNDERSCORE)));
        }
        case ANDNODE:
        ...
        case DOTNODE: {
            ...
            RubySymbol symbolID = symbolID(lexer, label);
    ...
    private Node getFlipConditionNode(Node node) {
        ...
        if (node instanceof FixnumNode) {
            ...
            return getOperatorCallNode(lexer, node, EQ_EQ, new GlobalVarNode(node.getLine(), symbolID(lexer, DOLLAR_DOT)));
        } 
    ...
    public SplatNode newSplatNode(Node node) {
        return new SplatNode(getLine(node), makeNullNil(node));
    }
    
    public ArrayNode newArrayNode(int line, Node firstNode) {
        ArrayNode array = new ArrayNode(line);

        array.add(makeNullNil(firstNode));
    ...
    */
    ...
    public int getLine(Node node) {
        return node == null ? lexer.getRubySourceline() : node.getLine();
    }
    ...
    private int[] allocateNamedLocals(RegexpNode regexpNode) {
        ...
        RubyRegexp pattern = RubyRegexp.newRegexp(runtime, regexpNode.getValue(), regexpNode.getOptions());
        pattern.setLiteral();
        ByteList[] names = pattern.getNames();
        int[] locals = new int[names.length];
        StaticScope scope = getCurrentScope();
        int actualIndex = 0;

        for (ByteList name: names) {
            if (lexer.isReservedWord(name.unsafeBytes(), name.begin(), name.realSize()) == null &&
                ...
                if (slot >= 0) {
                    ...
                    if (warnings.isVerbose() && !scope.isNamedCapture(slot)) {
                        warnings.warn(ID.AMBIGUOUS_ARGUMENT, lexer.getFile(), getLine(regexpNode), str(runtime, "named capture conflicts a local variable - " , symbol));
                    }
    ...
    public void compile_error(String message) { // mri: rb_compile_error_with_enc
        ...
        String errorMessage = lexer.getFile() + ":" + (lexer.getLine() + 1) + ": ";
    ...
    public static Node newRegexpNode(RubyLexer lexer, Node contents, RegexpNode end) {
        ...
        if (contents == null) {
            ...
            return new RegexpNode(lexer.getLine(), newValue, options.withoutOnce());
...
//                    if (!lexer.isOneEight()) encoding = frag.getEncoding();
        ...
        node.add(new StrNode(line, master));
        node.add(contents);
        return node;
    }
